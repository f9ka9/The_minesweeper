...
void MainWindow::on_new_game() {
    ...
    for (int r = 0; r < rows; ++r) {
        for (int c = 0; c < cols; ++c) {
            cellRects[r][c]->set_fill_color(Color::green);
    ...
    if (cell.isRevealed) {
        ...
        if (cell.isMine) {
            ...
            switch(cell.neighboringMines) {
                ...
                default: text->set_color(Color::black); break;
            }
        } else text->set_label("");
    } else {
        rect->set_fill_color(cell.isFlagged ? Color::red : Color::green);
        text->set_label("");
    }
}
...
void MainWindow::clickCellAtIndex(int r, int c, bool rightClick) {
    ...
    if (exploded) {
        ...
        gameOver = true;
        statusText->set_label("Статус: \nПроигрыш");
        drawBoard();
        fl_message("Вы проиграли!");
        return;
    }
...
int MainWindow::handle(int event) {
    switch(event) {
        case FL_PUSH: {
            ...
            bool right = (button == FL_RIGHT_MOUSE);
            int r,c;
